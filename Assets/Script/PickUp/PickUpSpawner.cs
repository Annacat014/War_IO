       ...
       protected void Update()
        {
            if (_currentCount < _maxCount)
            {
                ...
                if (_currentSpawnTimeSeconds > _spawnIntervalSeconds)
                {
                    ...
                    var picUp = Instantiate(_pickUpPrefab, randomPosition, Quaternion.identity, transform);
                    picUp.OnPickeUp += OnItemPickedUp;
                }
            }
        }

        private void OnItemPickedUp(PickUpItem pictedUpItem)
        {
            _currentCount--;
            pictedUpItem.OnPickeUp -= OnItemPickedUp;
        }

        protected void OnDrawGizmos()
        {
            var casherColor = Handles.color;
